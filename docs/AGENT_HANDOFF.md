...
| **Claude** | a comment on any PR that Claude is subscribed to (Claude subscribes to every open PR), or the owner in Claude's session | this Claude Code cloud session | designs and codes, opens and merges PRs, and posts as `mgalic01` with the footer "Generated by Claude Code" | the owner's Claude plan |
